<?
if(IsModuleInstalled('coffeediz.startup'))
{
	if (is_dir(dirname(__FILE__).'/install/components'))
		$updater->CopyFiles("install/components", "/local/components/");

	if (is_dir(dirname(__FILE__).'/install/js'))
		$updater->CopyFiles("install/js", "js/coffeediz.startup/");
	
	if (is_dir(dirname(__FILE__).'/install/wizards/coffeediz/Startup/site/templates/startup'))
		$updater->CopyFiles("install/wizards/coffeediz/Startup/site/templates/startup", "/local/templates/startup/");
	
	if (is_dir(dirname(__FILE__).'/install/wizards'))
		$updater->CopyFiles("install/wizards", "/bitrix/wizards/");
}
